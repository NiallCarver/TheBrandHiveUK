    ...
    <section id="process" aria-labelledby="process-title" className="py-20">
      <div className="container-slim space-y-8">
        <div>
          ...
          <p className="p mt-4 max-w-3xl">{introBody}</p>
        </div>
          ...
          {steps.map((s, i) => (
            <Reveal key={s.title} delay={0.04 * i}>
              <div className="card ui-glow p-6 h-full">
                ...
                <div className="mt-4 h-28 rounded-lg border border-white/10 bg-white/5 flex items-center justify-center text-xs text-muted">
                  Infographic / graph placeholder
                </div>
              </div>
            </Reveal>
          ))}
